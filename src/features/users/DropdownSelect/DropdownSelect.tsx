...
import {useBanUserMutation, User, useRemoveUserMutation, useUnBanUserMutation} from "@/generated/graphql";
...
const DropdownSelect = ({user, refetch}: Props) => {
    ...
    const {openModal: openDeleteModal, isOpen: deleteIsOpen, closeModal: closeDeleteModal} = useModal()
    ...
    return (
        <>
            ...
            <InfoModal modalTitle={"DELETE USER "} onClose={closeDeleteModal} open={deleteIsOpen}>
                <Typography variant={"body2"}>Are you sure you want to
                    delete {user.userName}?</Typography>
                <Button onClick={() => userDelete(user.id)}>YES</Button>
                <Button onClick={closeDeleteModal}>NO</Button>
            </InfoModal>

            <InfoModal modalTitle={"BAN USER "} onClose={closeBanModel} open={isBanOpen}>
                {user.userBan ? (<Typography>Are you sure want to un-ban {user.userName}?</Typography>) : (
                    <Typography variant={"body2"}>Are you sure you want to
                        ban {user.userName}?</Typography>)}
                {!user.userBan && (
                    <div>
                        <SelectCustom
                            options={options}
